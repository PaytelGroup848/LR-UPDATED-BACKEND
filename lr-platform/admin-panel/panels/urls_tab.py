...
class UrlsTab(ttk.Frame):
    ...
    def _build(self):
        ...
        button(heading, '+  Generate Portal URL', self.generate, SUCCESS).pack(side=tk.RIGHT, pady=2)
        ...
        tk.Label(
            form,
            text=(
                'Generate a reusable company-branded login page URL. '
        ...
        self.output.insert('1.0', 'Generate the company login portal URL to copy it here.')
    ...
    def generate(self):
        ...
        if not self.app.require_login():
            return
        try:
            data = self.app.client.generate_portal_url()
            url = data.get('url', '')
            ...
            self.output.insert(
                ...
                'Login: Each company user enters their own username and password.\n'
                'This reusable portal URL does not auto-login any user.'
            )
            self.output.configure(state=tk.DISABLED)
            try:
                self.links = self.app.client.login_links(limit=100)
